...
argument-hint: "(optional) absolute or repo-relative path to a Godot project root"
...
Treat `$ARGUMENTS` as the absolute path to a Godot project root (the directory containing `project.godot`). If the user did not supply one, ask which project they want inspected — do **not** guess a default.
...
```powershell
pwsh ./tools/automation/invoke-scene-inspection.ps1 -ProjectRoot "<project-root>"
```
