...
- Read `RUNBOOK.md` before selecting a harness script. Never read addon source files (`addons/agent_runtime_harness/`) to understand the harness protocol; all agent-facing contracts are in `RUNBOOK.md`, `docs/runbook/`, and `specs/008-agent-runbook/contracts/`.
...
- For runtime verification from this repository checkout, prefer the parameterized orchestration scripts (`tools/automation/invoke-*.ps1`); fall back to `tools/automation/get-editor-evidence-capability.ps1` and `tools/automation/request-editor-evidence-run.ps1` only when needed.
