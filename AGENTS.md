...
- For runtime verification in this repository, prefer the parameterized orchestration scripts: `pwsh ./tools/automation/invoke-input-dispatch.ps1`, `invoke-scene-inspection.ps1`, `invoke-build-error-triage.ps1`, `invoke-runtime-error-triage.ps1`, or `invoke-behavior-watch.ps1`. Consult `RUNBOOK.md` for the matching script, fixture template, and recipe. Fall back to `pwsh ./tools/automation/get-editor-evidence-capability.ps1` and `pwsh ./tools/automation/request-editor-evidence-run.ps1` only when the invoke scripts cannot satisfy the need, then inspect the persisted bundle manifest first.
- **Evidence lifecycle**: the transient zone is automatically cleared before every run — you never need to delete files. Zone classification is defined by `Get-RunZoneClassification` in `RunbookOrchestration.psm1` and documented in `specs/009-evidence-lifecycle/data-model.md`. To preserve a run across cleanups, pin it with `invoke-pin-run.ps1` before the next invocation. To compare two runs, pin both and reference them by name; never reconstruct a prior run from stale transient-zone files. Lifecycle scripts (`invoke-pin-run.ps1`, `invoke-unpin-run.ps1`, `invoke-list-pinned-runs.ps1`) emit `specs/009-evidence-lifecycle/contracts/lifecycle-envelope.schema.json`-conformant envelopes.
