...
| "Verify at runtime," "test the running code," "make sure the node appears in game," or another runtime-visible claim | Scenegraph Harness runtime verification | `/godot-inspect` (Claude Code) or `invoke-scene-inspection.ps1` (other tools) | The task needs a brokered run and persisted runtime evidence. |
...
| Drive deterministic keyboard or input-action events through the running game | Scenegraph Harness runtime verification with an `inputDispatchScript` | `/godot-press` or `invoke-input-dispatch.ps1` | Single-step skill / script handles capability gate, request, poll, and outcome assembly. |
| Sample a node property or signal over a frame window | Scenegraph Harness runtime verification with a `behaviorWatchRequest` | `/godot-watch` or `invoke-behavior-watch.ps1` | Returns `samplesPath`, `sampleCount`, and `frameRangeCovered` in the stdout envelope. |
| Diagnose a GDScript or resource build failure | Build-error triage | `/godot-debug-build` or `invoke-build-error-triage.ps1` | Returns `failureKind = "build"` with `firstDiagnostic` and optional `rawBuildOutputPath`. |
| Capture runtime errors and warnings from a running playtest | Scenegraph Harness runtime verification | `/godot-debug-runtime` or `invoke-runtime-error-triage.ps1` | `runtime-error-records.jsonl` is emitted automatically. Returns `latestErrorSummary` and `terminationReason` in the envelope. |
