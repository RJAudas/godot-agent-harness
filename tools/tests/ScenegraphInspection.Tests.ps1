Describe 'specs/002-inspect-scene-tree scenegraph snapshot schema' {
    ...
    It 'accepts the healthy snapshot fixture' {
        $result = Invoke-RepoJsonScript -ScriptPath 'tools/validate-json.ps1' -Arguments @(
            '-InputPath', 'examples/pong-testbed/harness/expected-live-scenegraph.json',
    ...
    It 'accepts a manual capture variant of the healthy snapshot fixture' {
        ...
        $snapshot = Get-Content -LiteralPath (Get-RepoPath -Path 'examples/pong-testbed/harness/expected-live-scenegraph.json') -Raw | ConvertFrom-Json -Depth 100
...
Describe 'specs/002-inspect-scene-tree scenegraph diagnostics schema' {
    ...
    It 'accepts the missing-node diagnostics fixture' {
        $result = Invoke-RepoJsonScript -ScriptPath 'tools/validate-json.ps1' -Arguments @(
            '-InputPath', 'examples/pong-testbed/evidence/scenegraph-diagnostics.json',
...
Describe 'scenegraph evidence manifest integration' {
    ...
    It 'adds scenegraph artifact kinds when they exist in the runtime artifact directory' {
        ...
        try {
            ...
            Copy-Item -LiteralPath (Get-RepoPath -Path 'examples/pong-testbed/evidence/scenegraph-snapshot.json') -Destination (Join-Path $runtimePath 'scenegraph-snapshot.json')
            Copy-Item -LiteralPath (Get-RepoPath -Path 'examples/pong-testbed/evidence/scenegraph-diagnostics.json') -Destination (Join-Path $runtimePath 'scenegraph-diagnostics.json')
            Copy-Item -LiteralPath (Get-RepoPath -Path 'examples/pong-testbed/evidence/scenegraph-summary.json') -Destination (Join-Path $runtimePath 'scenegraph-summary.json')
    ...
    It 'rejects unsupported artifact kinds' {
        ...
        $manifest = Get-Content -LiteralPath (Get-RepoPath -Path 'examples/pong-testbed/harness/expected-evidence-manifest.json') -Raw | ConvertFrom-Json -Depth 100
